...
% tufte-book's group manipulation during \chapter* output prevents a locally-
% defined \item from being visible when the .nls items are processed.  Fix:
% globally redirect \item through a flag so group scope is irrelevant.
\makeatletter
...
% Nomenclature item handler -- delimited [#1] matches \item [{KEY}] in .nls
\def\@nom@nomitem[#1]{%
  \par\noindent
...
% When outside nomenclature the original behaviour is preserved exactly.
% Must use \gdef (global) because tufte-book's \chapter* creates groups that
% would otherwise restore \item to the original list definition before the
% .nls items are processed, causing "Lonely \item" errors.
...
% Set/clear the flag globally so it survives any group boundaries.
\renewcommand{\thenomenclature}{\global\@innomenclaturetrue}
\renewcommand{\endthenomenclature}{\global\@innomenclaturefalse\par}
\makeatother
\printnomenclature
